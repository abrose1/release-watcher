...
SPOTIFY_REFRESH_TOKEN         # Used for Spotify recommendations in discovery job
TMDB_API_KEY
...
Two auth modes:
1. **Client credentials** — for artist album lookups (no user data needed)
2. **Refresh token** — for recommendations API (uses `SPOTIFY_REFRESH_TOKEN` to get a fresh access token)

Note: The refresh token is generated by the `taste-profile` repo's intake script via a **manual copy-paste OAuth flow** (Spotify does not allow `http://localhost` redirect URIs; the user authorized via browser, copied the failed redirect URL from the address bar, and pasted it into the intake script). The release-watcher never needs to run the OAuth flow itself — it only uses the already-obtained refresh token to get fresh access tokens.

Key methods:
- `get_artist_albums(spotify_id, after_date)` → list of albums since date
- `get_artist_new_singles(spotify_id, after_date)` → list of singles since date (Tier 1 only)
- `get_recommendations(seed_artist_ids, limit=20)` → list of recommended tracks/artists (uses refresh token auth)
- `refresh_access_token(refresh_token)` → returns fresh access token
...
### Music Discovery
1. Get top 5 music artists by score from `tracked_creators`
2. Call `spotify.get_recommendations(seed_artist_ids)`
3. Filter out artists already in `tracked_creators` or `discovery_sent`
4. For each candidate: fire Brave Search, call `judge_discovery_candidate()`
5. Send top 1–2 results that pass the judge
...
**No env vars required to run the test suite.** Tests that need a DB use SQLite in-memory via a pytest fixture that creates and tears down all tables. Never use `DATABASE_URL` or any real connection string in tests — the test DB is always constructed from the SQLAlchemy models directly against SQLite.
...
- In `sync_creators.py`: `# TODO: connect to TASTE_PROFILE_DATABASE_URL and read book_authors + music_artists + profile_metadata`
...
The agent's work is complete when all of the following pass **with no credentials, no .env file, and no network access**:

- [ ] All files in the directory structure exist
- [ ] `pytest tests/ -v` passes — this is the primary verification method
- [ ] Test coverage is >80% on `watcher/judge.py`, `watcher/notify.py`, and `watcher/sync_creators.py`
- [ ] The test suite includes at least one integration-style test per job (`watchlist`, `announcement`, `discovery`) that runs the full job flow against a SQLite in-memory DB with mocked API clients and a mocked judge
...
- [ ] Alembic migration exists and is consistent with the schema defined in `models.py`

The `--dry-run` and `--stub` flags are for the human's use after wiring up real credentials — the agent does not need to invoke them to verify completeness.
